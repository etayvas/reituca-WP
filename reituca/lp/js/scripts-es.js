...
// Validate Name (no special chars + no empty)
$('#name').plum('form.verify',function () {	
...
// regex for countries
$('#countries').plum('form.verify', function () {
    if ( $('#countries').val() != "Select a country")
        return true;});

// regex for phone
$('#phone').plum('form.verify', function () {	
    return	/^[\d-]{6,16}$/.test(this.value); });
    
// regex for Country Code
$('#country_code').plum('form.verify', function () {	
    return	/^[\d-]{1,4}$/.test(this.value); });
...
jQuery(document).ready(function() { 
	jQuery('#countries').on('change', function() { updateCountryCode(); });
	updateCountryCode();
});
        
var updateCountryCode = function() {
	var countryCode = jQuery('#countries option:selected').attr('cnCode');
	jQuery('.c_code_class').val(countryCode);
}
//$('.c_code_class').prop('readonly', true);
/*
// pop up window
$('#body').click(function() {
  //  alert($('.email .info').hasClass('error'));
  $printError = '';
  $emailError = 'Please check your email';
  $nameError = 'Please check your name';
    if ($('.email .info').hasClass('success'))
        $emailError = '';
    //else $emailError ='';
    
    if ($('.name .info').hasClass('success'))
        $nameError = '';        
    //else $nameError = '';
});
$('.submit').click(function() {
    $printError = $emailError + $nameError;
    alert($printError);
});

  //  $testUN = $("#name").val();
    //$para = "{'userName':'" + $testUN + "'}";

*/
//$.validator.addMethod("check_country", function(value, element) {
//   return $('#countries').val() == 'Select a country'
 ...
 $("#myform").validate({
   ...
   messages: {
     name: {
       ...
       minlength: jQuery.format(" Por favor, ingrese un nombre válido.")

     },
     email: {
       required: " Por favor, ingrese un dirección de correo electrónico válida.",
       email: " Por favor, ingrese un dirección de correo electrónico válida."
     },
     phone: {
       required: " Por favor, ingrese un número de teléfono válido",
       digits: " Por favor, ingrese un número de teléfono válido.",
       rangelength: jQuery.format(" Por favor, ingrese un número de teléfono válido.")
     },
     country_code: {
       required: " ",
